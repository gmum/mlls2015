...
def joblib_check(key, cache_dir):
    return len(glob.glob(os.path.join(cache_dir, key + "*"))) > 0

def joblib_save(key, val, cache_dir):
    file_name = os.path.join(cache_dir, key)
    joblib.dump(val, file_name)

def mmap_joblib_load(key, cache_dir):
    file_name = os.path.join(cache_dir, key)
    return joblib.load(file_name, mmap_mode="r")
...
def numpy_save_fnc(key, val, cache_dir):
    ...
    if isinstance(val, list):
        save_path = os.path.join(cache_dir, key)
        save_dict = {}
        for id, ar in enumerate(val):
            save_dict[str(id)] = ar
        np.savez(save_path, **save_dict)
    else:
        np.save(os.path.join(cache_dir, key + ".npy"), val)
...
def mmap_numpy_load_fnc(key, cache_dir):
    if os.path.exists(os.path.join(cache_dir, key + ".npz")):
        # Listed numpy array

        savez_file = np.load(os.path.join(cache_dir, key + ".npz"), mmap_mode="r")
        ...
        return np.load(os.path.join(cache_dir, key + ".npy"), mmap_mode="r")
