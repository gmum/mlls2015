...
def load_dataset(which, version, organism, protein):
    def _get_path(filename):
        return os.path.join(CONFIG['data_dir'], "datasets", '/'.join([which, version, organism, protein, filename]))

    X = pd.read_csv(_get_path("X.csv"), index_col=0, na_values=[], keep_default_na=False)
    y = pd.read_csv(_get_path("y.csv"), index_col=0, na_values=[], keep_default_na=False)
    labels = pd.read_csv(_get_path("labels.csv"), index_col=0, na_values=[], keep_default_na=False)
    meta = pd.read_csv(_get_path("meta.csv"), index_col=0, na_values=[], keep_default_na=False)
    with open(_get_path("splits.json"), 'r') as f:
        splits = json.load(f)
...
def load_smiles():
    path = os.path.join(CONFIG['data_dir'], "datasets", "features", "smiles.csv")
    return pd.Series.from_csv(path, index_col=0)


def load_fingerprint(which):
    path = os.path.join(CONFIG['data_dir'], "datasets", "features", which + "_v1.libsvm")
    return load_svmlight_file(path)[0]
...
for protein in PROTEINS:
    ...
    molprint_data = load_fingerprint("MolPrint2D")
    ...
    for fingerprint in FINGERPRINTS:
        ...
        # download data
        X_all, y_all, labels, meta_all, splits_all = load_dataset(which="log_Ki_basic",
                                                                  version="v1",
                                                                  organism="Human",
                                                                  protein=protein)

        smiles = load_smiles()
        fp_data = load_fingerprint(fingerprint)

        y = y_all["log_Ki_thr:(2.5,)"].values
        X = fp_data[get_idx(X_all.index, list(smiles.index.values)), :]

        relative_ids = get_idx(X_all.index.tolist(), list(smiles.index.values))
        all_chembl_ids = list(smiles.index.values)

        def _reindex(chembl_ids):
            """
            get a set of chemblID, for each get its id (global for all proteins)
            then reidex it to relative for a given protein
            """
            reids = []
            for id in chembl_ids:
                all_id = all_chembl_ids.index(id)
                reids.append(relative_ids.index(all_id))
            return reids
        ...
        for fold in [str(i) for i in xrange(5)]:
            ...
            splits[fold] = {'train': _reindex(train_uids),
                            'test': _reindex(test_uids)}
